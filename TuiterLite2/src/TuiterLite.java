...
public class TuiterLite<T> {
     ...
     */
    public Tuite tuitarAlgo(Usuario usuario, String texto) throws UsuarioDesconhecidoException, IllegalArgumentException, EmptyStringException, TamanhoMaximoExcedidoException {
        ...
        if(texto.length() > TAMANHO_MAXIMO_TUITES){
            throw new TamanhoMaximoExcedidoException(texto);
        }
        if(!usuarioByEmail.containsKey(usuario.getEmail())){
            throw new UsuarioDesconhecidoException(usuario.getEmail());
        }
